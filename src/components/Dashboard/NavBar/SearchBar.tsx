...
export const SearchBar = ({ onSearchClick, className }: SearchBarProps) => {
    ...
    return (
        <div 
            ref={dropdownRef}
            className={cn(
                "relative flex flex-row justify-between bg-light-gray border rounded-3xl w-[314px] h-[40px] pl-5 pr-2 items-center",
                className
            )}
        >
            <button
                    ...
                    >
                        {tags.map(tag => (
                            <div
                                ...
                                className="capitalize p-3 hover:bg-white rounded-lg duration-200 cursor-pointer text-[11px]"
                            >
                                {tag}
                            </div>
                        ))}
