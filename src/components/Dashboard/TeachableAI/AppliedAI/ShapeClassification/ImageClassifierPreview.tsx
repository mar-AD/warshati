...
import { PredictedClass } from '../Classification';
...
const ImageClassifierPreview = () => {
  ...
  const t = useTranslations("image_classifier_preview");
  const t2 = useTranslations("step_navigation");
  ...
  const getPredictedClass = async (dataURL: string) => {
    ...
    const data = await sendPostRequest(`/shape/classify/${sessionId}`, { image: dataURL });
  ...
  return (
    <div className="h-5/6 px-4 md:px-10">
