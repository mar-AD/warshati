...
import bubble from "/public/images/SmartWorkshopsProgram/bubble.png";
import robot from "/public/images/Home/curricula/robot_3.png"
...
return (
    <div
        className={` bg-light-gray lg:px-[7.25rem] py-20 px-5 font-Poppins gap-10 relative flex items-center justify-between overflow-hidden
    ...
    >
        ...
        >
            <div className={`relative w-full h-full flex ${isExtraSmallScreen || isSmallScreen ? "justify-start place-items-end" : "justify-left items-end"}`}>
                <Image 
                    className={`max-w-none ${isExtraExtraLargeScreen ? "!w-[430px]" : "!w-[400px]"}`}  
                    src={robot}  
                    alt="" 
                />
                ...
                <div className={`absolute border border-violet-600 rounded-full 
                    ${isExtraSmallScreen ? "top-[15%] right-[12%]" : ""}
                    ${isSmallScreen && !isExtraSmallScreen ? "top-[21%] right-[20%]" : ""}
                    ${isMediumScreen && !isSmallScreen ? "top-[21%] right-[10%]" : ""}
                    ${isLargeScreen && !isMediumScreen && !isSmallScreen ? "top-[21%] right-[15%]" : ""}
                    ${isExtraLargeScreen && !isLargeScreen ? "top-[25%] right-[17%]" : ""}
                    ${isExtraExtraLargeScreen ? "top-[25%] right-[19%]" : ""}
                `}>
