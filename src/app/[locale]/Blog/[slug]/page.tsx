...
const ArticlePage = () => {
  const { slug } = useParams<{ slug: string }>();
const t = useTranslations("blog.rest");
const tArticles = useTranslations("blog");

const sharedArticle = articleData.find((a) => a.slug === slug);
const translatedArticles = tArticles.raw("articles") as Record<string, Article>;
const translatedArticle = translatedArticles[slug];
    ...
    <div className="space-y-8">
      ...
      <div className="lg:w-full overflow-hidden">
        <Image
          src={article.image}
          alt={article.title}
          className={article.slug === articleData[0].slug ? "w-[53rem] h-[444px]" : "w-full h-[420px]"}
          width={848}
          height={420}
        />
      </div>
